...
echo "运行 Termux:API 分享卡住回归：应超时后自动切到系统下载服务"
...
setup = subprocess.run(
    docker_command + [
        ...
        r'''
        set -euo pipefail
        rm -rf /data/data/com.termux/files/home/storage /data/data/com.termux/files/home/.sillytavern
        mkdir -p /tmp/export-api-bin
        cat > /tmp/export-api-bin/termux-share <<'EOF'
#!/usr/bin/env bash
sleep 60
EOF
        cat > /tmp/export-api-bin/termux-download <<'EOF'
#!/usr/bin/env bash
printf "%s\n" "$*" > /tmp/termux-download-args.log
EOF
        cat > /tmp/export-api-bin/python3 <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
if [[ "${1:-}" == "-" ]]; then
    cat >/dev/null
...
exit 2
EOF
        chmod +x /tmp/export-api-bin/termux-share /tmp/export-api-bin/termux-download /tmp/export-api-bin/python3
''',
    ],
)
...
command = docker_command + [
    ...
    set -euo pipefail
    cd /workspace
    env HOME=/data/data/com.termux/files/home PATH="/tmp/export-api-bin:/usr/local/bin:/usr/bin:/bin" SILLYDROID_EXPORT_SHARE_TIMEOUT_SECONDS=1 /opt/export-tavern-data.sh --install-root /workspace/SillyTavern
''',
]

# 分享面板回退确认必须走 /dev/tty，覆盖 curl | bash 中 stdin 被占用的真实路径。
...
try:
    ...
    while True:
        if time.time() > deadline:
            process.kill()
            raise TimeoutError("share timeout fallback test timed out")
        ...
        if not sent_yes and ("是否使用系统分享面板导出？" in text or "回退方案：使用 Android 系统分享面板" in text):
        ...
        if process.poll() is not None:
            ...
            required = [
                "Android 系统分享面板",
                "调用 termux-share 失败或超时",
                "将继续尝试 Android 系统下载服务",
                "压缩包已交给系统下载服务",
            ]
            for needle in required:
                if needle not in text:
                    raise AssertionError(f"missing fallback text: {needle}")
            break
finally:
    os.close(master_fd)
...
check = subprocess.run(
    docker_command + [
        ...
        r'''
        set -euo pipefail
        grep -q "SillyTavern 数据备份" /tmp/termux-download-args.log
        test "$(find /data/data/com.termux/files/home/.sillytavern/export-cache -maxdepth 1 -type f -name "sillytavern-termux-backup-*.zip" | wc -l)" -eq 1
''',
    ],
)
